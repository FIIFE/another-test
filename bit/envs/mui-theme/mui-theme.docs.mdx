...
This environment can be applied to all your components or a set of components via the command `bit env set <pattern or component_id> fifi.bit/envs/mui-theme`, or under the variants of your `workspace.json` file.
...
Run the command `bit env set <"pattern" or component_id> fifi.bit/envs/mui-theme` where the pattern is a glob pattern to select multiple components. This will put the env
...
#### Alternative Usage
Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `mui-theme.aspect.ts` file.
...
Bit uses ESLint to lint your components. You can add your own rules in the `mui-theme.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `mui-theme.main.runtime` file.
...
Override the default dependencies in the `mui-theme.main.runtime` file to include react types of a different version for example.
...
Extend the `mui-theme.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
